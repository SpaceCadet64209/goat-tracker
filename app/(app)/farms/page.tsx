  ...
  return (
    <section className="mx-auto max-w-xl rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <p className="mt-2 text-sm leading-6 text-stone-600">
        Select the farm you want to work in. Each link opens an explicit farm
        context.
      </p>
      <ul className="mt-6 divide-y divide-stone-200 rounded-xl border border-stone-200">
        {farms.map((farm) => (
          <li key={farm.id}>
            <Link
              href={`/farms/${farm.id}`}
              className="flex min-h-14 items-center justify-between gap-4 px-4 font-semibold text-stone-900 hover:bg-stone-50 focus:ring-2 focus:ring-emerald-700 focus:outline-none focus:ring-inset"
            >
              <span>{farm.name}</span>
              <span className="rounded-full bg-stone-100 px-2 py-1 text-xs font-medium text-stone-600 capitalize">
                {farm.role}
              </span>
            </Link>
          </li>
