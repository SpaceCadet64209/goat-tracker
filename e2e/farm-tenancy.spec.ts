...
test.describe("farm tenancy journeys", () => {
  ...
  test("farm entry sends a member to an explicit farm route", async ({
    page,
  }) => {
  ...
  test("an unauthorized farm URL reveals no protected farm content", async ({
    page,
  }) => {
  ...
  test("farm switching remains usable at a mobile viewport", async ({
    page,
  }) => {
...
test.describe("farm onboarding", () => {
  ...
  test("a user without farms can create their first owner farm", async ({
    page,
  }) => {
    await page.goto("/farms");
    await expect(
